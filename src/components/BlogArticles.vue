<template>
  <div class="main__aboutMe blogId">
    ...
    <p class="blogId__info">{{ `${articlesAuthor.firstName} ${articlesAuthor.lastName}` }}</p>
    <p class="blogId__info">{{ timed() }}</p>
  </div>
</template>
...
export default {
  props: {
    ...
    articlesAuthor: Object
  },
  setup(props) {
    const timed = () => {
      const blogDate = new Date(props.artilesTime)
